...
现代 Web 应用应优先使用更安全的 API，例如参数化查询、自动转义机制或模板框架。如果无法使用这些机制，就必须在把数据交给目标解释器之前，根据目标上下文进行编码、转义或净化，才能保证应用安全。
...
| **1.1.2** | 验证数据在交给目标解释器之前的最后一个处理步骤，才根据目标上下文进行编码或转义；也可以由目标解释器自行安全处理。 | 2 |

## V1.2 注入预防

本章所说的“输出编码或转义”，是指应用把数据交给浏览器、数据库、命令行等下游解释器之前，根据目标上下文进行安全处理，而不是对解释器运行后产生的结果进行编码。此类处理应尽可能接近数据进入目标上下文的时刻。通常不应长期保存编码或转义后的结果，而应在数据即将交给相应解释器时再处理。处理得过早，既可能破坏内容格式，也可能使编码或转义失去防护作用。
...
| **1.2.5** | 验证应用能够防范操作系统命令注入。调用操作系统命令时，应使用能够将命令与参数分离的参数化接口；如果无法使用，则必须在将数据传给命令行解释器之前，按照相应的命令行上下文进行转义或编码。 | 1 |
