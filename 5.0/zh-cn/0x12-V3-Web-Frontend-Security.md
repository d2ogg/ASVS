...
| **3.3.2** | 验证每个 Cookie 都根据自身用途设置了合适的 `SameSite` 属性值，以降低界面跳转攻击和浏览器请求伪造攻击（通常称为跨站请求伪造，即 CSRF）的风险。 | 2 |
| **3.3.3** | 验证 Cookie 名称使用 '__Host-' 前缀，除非这些 Cookie 被明确设计为与其他主机共享。 | 2 |
| **3.3.4** | 验证不应由客户端脚本读取的 Cookie（例如存放会话令牌的 Cookie）设置了 `HttpOnly` 属性。同一个敏感值只能通过 `Set-Cookie` 头发送给客户端，不得通过其他响应内容传输。 | 2 |
...
| **3.4.2** | 验证应用把跨源资源共享（CORS）的 Access-Control-Allow-Origin 响应头设置为固定值。如果该值取自请求中的 Origin 头，则必须先对照可信来源允许列表进行验证。必须使用 `Access-Control-Allow-Origin: *` 时，响应中不得包含任何敏感信息。 | 1 |
| **3.4.3** | 验证 HTTP 响应包含 Content-Security-Policy 头，并通过其中的指令限制浏览器只能加载和执行可信内容或资源，从而限制恶意 JavaScript 的执行。最低要求是制定全局策略：包含 `object-src 'none'` 和 `base-uri 'none'`，并为其他资源定义允许列表，或者采用 nonce 或哈希。L3 应用必须为每个响应生成带 nonce 或哈希的独立策略。 | 2 |
...
| **3.4.6** | 验证 Web 应用在每个 HTTP 响应的 Content-Security-Policy 头中设置了 `frame-ancestors` 指令。默认情况下应禁止其他页面嵌入应用内容；确有需要时，只允许指定的页面嵌入。虽然浏览器仍支持 X-Frame-Options，但该头已经过时，不应再依赖它。 | 2 |
| **3.4.7** | 验证 Content-Security-Policy 头指定了策略违规报告的接收位置。 | 3 |
| **3.4.8** | 验证所有会触发文档渲染的 HTTP 响应（例如 Content-Type 为 `text/html` 的响应）根据需要设置了 Cross-Origin-Opener-Policy 头，并使用 `same-origin` 或 `same-origin-allow-popups` 指令。这样可以防止攻击者滥用不同页面对 `Window` 对象的共享访问，例如实施反向标签劫持（tabnabbing）或框架计数（frame counting）攻击。 | 3 |
...
主要防护机制包括浏览器的同源策略（针对 JavaScript）和 Cookie 的 SameSite 规则。CORS 预检也是常见的防护手段：对允许跨源调用的端点，它是重要的访问控制环节；对本来不应跨源调用的端点，它也能帮助阻止伪造请求。
...
| **3.5.8** | 验证只有在业务确实需要时，才允许以用户身份加载或嵌入需要认证的资源，例如图片、视频、脚本和其他文档。可以严格检查 `Sec-Fetch-*` 请求头，排除非预期的跨源调用；也可以设置严格的 Cross-Origin-Resource-Policy 响应头，让浏览器阻止不符合策略的内容。 | 3 |
...
| # | 描述 | 级别 |
| :---: | :--- | :---: |
| **3.6.1** | 验证 JavaScript 库、CSS、Web 字体等客户端资源只有同时满足以下条件时，才托管在外部站点（例如 CDN）：资源是静态的、带有明确版本，并通过子资源完整性（SRI）校验其完整性。无法满足这些条件时，必须逐项记录安全决策及理由。 | 3 |
