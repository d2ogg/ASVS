...
| **4.1.5** | 验证高度敏感或需要经过多个系统的请求和业务操作采用了逐消息数字签名，在传输层保护之外进一步保证消息的真实性和完整性。 | 3 |
...
| **4.2.5** | 验证应用的后端或前端在构造并发送请求时，会通过验证、净化或其他机制限制 URI（例如 API 调用地址）和 HTTP 请求头字段（例如 `Authorization` 或 `Cookie`）的长度，避免接收方因内容过长而拒绝请求。否则，应用可能陷入持续错误状态并造成拒绝服务；例如，客户端一旦保存了过长的 Cookie，后续请求便可能始终被服务器拒绝。 | 3 |
...
| # | 描述 | 级别 |
| :---: | :--- | :---: |
| **4.3.1** | 验证应用通过查询允许列表、查询深度限制、查询数量限制或查询成本分析，防止复杂的嵌套查询耗尽 GraphQL 服务或数据层资源并造成拒绝服务（DoS）。 | 2 |
...
| **4.4.2** | 验证初次进行 HTTP WebSocket 握手时，会对照应用允许的来源列表检查 Origin 头。 | 2 |
