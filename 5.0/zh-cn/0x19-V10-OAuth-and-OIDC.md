...
| **10.5.5** | 验证：当身份认证服务（OpenID Provider）通过发送注销令牌，通知接入 OIDC 登录的应用结束用户会话时，该应用必须防止攻击者伪造注销通知，强制用户退出登录并造成拒绝服务，同时还必须防止注销令牌与其他 JWT 混淆。该应用必须确认注销令牌的类型为 `logout+jwt`，其中包含成员名称正确的 `event` 声明，且不包含 `nonce` 声明。还建议将注销令牌的有效期设得较短，例如 2 分钟。 | 2 |
