...
| **10.4.10** | 验证：机密客户端发起令牌请求、推送授权请求（PAR）、令牌撤销请求等后通道请求时，会向授权服务器证明自身身份。 | 2 |
...
| **10.4.14** | 验证：授权服务器只签发发送方约束（持有证明）访问令牌，例如使用 mTLS 证书绑定访问令牌，或使用 DPoP 绑定访问令牌。 | 3 |
| **10.4.15** | 验证：服务端客户端不在终端用户设备上运行时，授权服务器能够确认 `authorization_details` 参数来自客户端后端，且未被用户篡改。可以强制采用推送授权请求（PAR）或 JWT 安全授权请求（JAR）来满足此要求。 | 3 |
...
| # | 描述 | 级别 |
| :---: | :--- | :---: |
| **10.5.1** | 验证：作为依赖方的客户端能够防范 ID Token 重放。例如，检查 ID Token 中的 `nonce` 是否与发送给 OpenID Provider 的认证请求中的 `nonce` 完全一致；在 OAuth 2.0 中，这类请求称为发送给授权服务器的授权请求。 | 2 |
...
| **10.5.5** | 验证：使用 OIDC 后通道注销时，依赖方能够防范攻击者强制注销用户所造成的拒绝服务，并防止注销令牌与其他 JWT 混淆。客户端必须确认注销令牌的类型为 `logout+jwt`，其中包含成员名称正确的 `event` 声明，且不包含 `nonce` 声明。还建议将令牌有效期设得较短，例如 2 分钟。 | 2 |
