...
| **10.1.2** | 验证客户端只接受与当前交易相匹配的授权服务器返回值，例如授权码或 ID Token。这些值必须来自同一用户代理会话发起的同一次授权流程。为此，客户端生成的 `code_verifier`、`state`、OIDC `nonce` 等秘密值必须不可猜测、仅供本次交易使用，并与客户端及发起交易的用户代理会话安全绑定。 | 2 |
...
| # | 描述 | 级别 |
| :---: | :--- | :---: |
| **10.3.1** | 验证资源服务器只接受以自身为预期受众的访问令牌。受众可以记录在结构化访问令牌中，例如 JWT 的 `aud` 声明；也可以通过令牌内省端点确认。 | 2 |
| **10.3.2** | 验证资源服务器依据访问令牌中描述委派权限的声明作出授权决定。令牌含有 `sub`、`scope`、`authorization_details` 等声明时，必须把这些声明纳入决策。 | 2 |
| **10.3.3** | 验证资源服务器需要从访问令牌（JWT 或令牌内省响应）识别唯一用户时，只采用不会被重新分配给其他用户的声明。通常应组合使用 `iss` 和 `sub`。 | 2 |
| **10.3.4** | 验证资源服务器对认证强度、认证方式或认证时效性有特定要求时，会确认访问令牌满足这些条件。例如，可以分别检查 OIDC 的 `acr`、`amr` 和 `auth_time` 声明（如有）。 | 2 |
| **10.3.5** | 验证资源服务器要求使用发送方约束访问令牌，防止他人使用或重放窃取的访问令牌。可以采用 OAuth 2.0 双向 TLS，或 OAuth 2.0 持有证明（DPoP）。 | 3 |
...
| **10.4.2** | 验证授权响应中的授权码只能成功换取一次令牌。如果同一授权码已经用于签发访问令牌，授权服务器必须拒绝后续令牌请求，并撤销与该授权码相关的所有已签发令牌。 | 1 |
...
| **10.4.5** | 验证授权服务器能够防范公共客户端中的刷新令牌重放。首选方案是使用受发送方约束的刷新令牌，例如采用持有证明（DPoP），或通过双向 TLS（mTLS）将令牌与证书绑定。L1 和 L2 应用也可以采用刷新令牌轮换：每个刷新令牌使用一次后必须立即失效；如果服务器再次收到已经使用并失效的刷新令牌，则必须撤销该次授权签发的所有刷新令牌。 | 1 |
| **10.4.6** | 验证授权服务器使用授权码流程时，强制采用授权码交换证明密钥（PKCE），以防范授权码截获。授权请求必须包含有效的 `code_challenge`，且 `code_challenge_method` 不得为 `plain`；令牌请求必须校验 `code_verifier`。 | 2 |
...
| **10.4.10** | 验证机密客户端发起令牌请求、推送授权请求（PAR）、令牌撤销请求等后通道请求时，会向授权服务器证明自身身份。 | 2 |
| **10.4.11** | 验证授权服务器只向 OAuth 客户端分配业务所需的 scope。 | 2 |
...
| **10.4.14** | 验证授权服务器只签发发送方约束（持有证明）访问令牌，例如使用 mTLS 证书绑定访问令牌，或使用 DPoP 绑定访问令牌。 | 3 |
| **10.4.15** | 验证服务端客户端不在终端用户设备上运行时，授权服务器能够确认 `authorization_details` 参数来自客户端后端，且未被用户篡改。可以强制采用推送授权请求（PAR）或 JWT 安全授权请求（JAR）来满足此要求。 | 3 |
...
| # | 描述 | 级别 |
| :---: | :--- | :---: |
| **10.5.1** | 验证作为依赖方的客户端能够防范 ID Token 重放。例如，检查 ID Token 中的 `nonce` 是否与发送给 OpenID Provider 的认证请求中的 `nonce` 完全一致；在 OAuth 2.0 中，这类请求称为发送给授权服务器的授权请求。 | 2 |
...
| **10.5.5** | 验证使用 OIDC 后通道注销时，依赖方能够防范攻击者强制注销用户所造成的拒绝服务，并防止注销令牌与其他 JWT 混淆。客户端必须确认注销令牌的类型为 `logout+jwt`，其中包含成员名称正确的 `event` 声明，且不包含 `nonce` 声明。还建议将令牌有效期设得较短，例如 2 分钟。 | 2 |
...
| **10.6.2** | 验证 OpenID Provider 能够防范利用强制注销造成的拒绝服务。可以要求最终用户明确确认注销；如果注销请求由依赖方发起，也可以检查其中的 `id_token_hint` 等参数。 | 2 |
...
| **10.7.2** | 验证授权服务器征求同意时，向用户清楚、完整地说明授权内容。适用时，应说明所请求权限的性质（通常由 scope、资源服务器和富授权请求（RAR）详情决定）、获得授权的应用身份，以及授权的有效期。 | 2 |
