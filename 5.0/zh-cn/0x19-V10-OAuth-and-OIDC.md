...
* 访问令牌只能由资源服务器（RS）接收并使用。它可以是通过内省进行验证的引用令牌，也可以是使用相应密钥材料验证的自包含令牌。
...
| # | 描述 | 级别 |
| :---: | :--- | :---: |
| **10.3.1** | 验证资源服务器在接受访问令牌前，会确认该令牌的预期接收方（受众）包含本资源服务器。受众信息可以记录在结构化访问令牌中，例如 JWT 的 `aud` 声明；也可以通过令牌内省端点确认。 | 2 |
