...
* **防伪令牌（Anti-forgery token）** – 随请求传递的一个或多个令牌。应用服务器通过验证令牌，确认请求来自预期端点。
* **应用安全（Application Security）** – 重点关注开放系统互连参考模型（OSI 模型）应用层组件的安全，而不包括底层操作系统、所连接网络等方面。
...
* **自动化验证（Automated Verification）** – 使用动态分析、静态分析或两者相结合的自动化工具，根据漏洞特征发现问题。
...
* **动态应用安全测试（Dynamic Application Security Testing，DAST）** – 在应用运行时检测可能表明存在安全漏洞的现象。
...
* **恶意代码（Malicious Code）** – 在应用所有者不知情的情况下，于开发期间被植入应用，并会绕过应用预期安全策略的代码。它不同于病毒、蠕虫等恶意软件。
...
* **引用令牌（Reference Token）** – 一种用作指针或标识符的令牌，指向存储在服务器上的状态或元数据，有时也称随机令牌或不透明令牌。自包含令牌会在令牌内嵌入相关数据，引用令牌则不携带这些信息，而要由服务器提供上下文。引用令牌本身可以是会话标识符，也可以包含会话标识符。
* **依赖方（Relying Party，RP）** – 通常指依靠独立认证提供方来认证用户的应用。应用根据认证提供方签发的令牌或一组签名断言，确认用户确实具有其所声明的身份。
...
* **单因素认证器（Single-factor authenticator）** – 仅使用一种认证因素来确认用户身份的机制。该因素可以是用户知道的信息（记忆秘密、密码、口令短语或 PIN）、用户自身的特征（生物识别信息，如指纹或面部特征），或用户持有的物品（OTP 令牌、智能卡等密码设备）。
...
* **无状态会话机制（Stateless Session Mechanism）** – 使用传递给客户端的自包含令牌来保存会话信息，接收并验证令牌的服务不一定另行存储这些信息。但在实际应用中，为了执行必要的安全控制，服务通常仍需访问部分会话信息，例如 JWT 撤销列表。
...
* **可信服务层（Trusted Service Layer）** – 任何可信的控制执行点，例如微服务、无服务器 API、服务端、具备安全启动能力的客户端设备上的可信 API，以及合作伙伴或外部 API。所谓可信，是指不可信用户无法绕过该层，也无法跳过该层实施的控制。
